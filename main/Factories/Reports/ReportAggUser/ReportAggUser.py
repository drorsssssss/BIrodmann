...
class ReportAggUser(IReporting):
    ...
    def execute(self):
        ...
        df = dfh.add_columns_pandas_df(df,agg_time_resolution,func)
        df = af.aggregate(df,[agg_time_resolution,SourceColumns.ID.value],af.AggBaseFunc.COUNT.value)
        df = cf.rename_pandas_cols(df,[agg_time_resolution,SourceColumns.ID.value,CalcColumns.COUNT.value])
        df = af.groupby_sort(df,agg_time_resolution,[CalcColumns.COUNT.value],ascending=False)
        df = af.groupby_limit(df,agg_time_resolution,topn)
